...
import { ChevronUp, ChevronDown, Trash2, Filter, Terminal, ArrowUp, ArrowDown, Copy, Check } from 'lucide-react';
          ...
          <div className="debug-console-logs" ref={logContainerRef}>
            {filteredLogs.length === 0 ? (
              ...
              filteredLogs.map((log) => {
                // Inline component to manage copy state per log entry
                return <LogEntry key={log.id} log={log} getLevelColor={getLevelColor} getLevelBadgeClass={getLevelBadgeClass} />;
              })
            )}
          </div>
        </div>
...
// Sub-component for individual log entries to manage local state (like copy feedback)
function LogEntry({ log, getLevelColor, getLevelBadgeClass }: { 
  log: DebugLogEntry, 
  getLevelColor: (l: LogLevel) => string, 
  getLevelBadgeClass: (l: LogLevel) => string 
}) {
  const [copied, setCopied] = useState(false);
  
  const handleCopy = (e: React.MouseEvent) => {
    e.stopPropagation();
    const text = `[${new Date(log.timestamp).toLocaleTimeString()}] [${log.level.toUpperCase()}] [${log.source}] ${log.message}${log.details ? `\nDetails: ${JSON.stringify(log.details, null, 2)}` : ''}`;
  ...
  return (
    <div className="debug-log-entry" onClick={() => log.details && console.log('Details:', log.details)}>
      ...
      <div style={{ display: 'flex', gap: '8px', marginLeft: 'auto', alignItems: 'center' }}>
        ...
        <button 
          ...
          style={{
            ...
            border: 'none',
            color: copied ? 'var(--accent-color)' : 'var(--text-tertiary)',
            cursor: 'pointer',
            padding: '2px',
            display: 'flex',
            alignItems: 'center',
