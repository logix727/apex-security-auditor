...
import { RepeaterTab } from './RepeaterTab';
import TriageTab from './TriageTab';
import { AiAnalysisModal } from './AiAnalysisModal';

import { ResponseDiff } from '../common/ResponseDiff';

export type InspectorTab = 'Summary' | 'Exchange' | 'Security' | 'Details' | 'Diff' | 'Repeater' | 'Triage';
...
const highlightFindings = (text: string, findings: Badge[], query: string) => {
    ...
    // Create regex that matches terms but allows flexible whitespace
    // Escape special chars first, then replace spaces with \s+ (one or more whitespace chars)
    // This allows multi-line matches in pretty-printed code
    const regexPattern = sortedTerms
    ...
    return (
      <aside className="inspector" style={{ gridArea: 'inspector', width: showInspector ? `${inspectorWidth}px` : '0px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '0 16px 16px 16px', gap: '16px' }}>
                    ...
                    <div style={{ flex: 1, minHeight: 0 }}>
                        {(() => {
                            ...
                            return (
                                <ResponseDiff
                                    old={baseScan?.response_body || ''}
                                    new={inspectorAsset.response_body || ''}
                                />
                            );
                        })()}
                    </div>
            ...
            {activeInspectorTab === 'Triage' && inspectorAsset && (
                <TriageTab inspectorAsset={inspectorAsset} />
            )}

            {activeInspectorTab === 'Repeater' && inspectorAsset && (
                <RepeaterTab inspectorAsset={inspectorAsset} activeView={activeView} />
            )}
        </div>
