...
impl OpenApiSpec {
    ...
    pub fn find_endpoint(&self, path: &str, method: &str) -> Option<&DocumentedEndpoint> {
        ...
        self.endpoints
            .iter()
            .find(|endpoint| endpoint.method == normalized_method && paths_match(&endpoint.path, path))
    }
}
...
/// # Example
/// ```rust,no_run
/// use tauri_appapex_security_auditor_lib::parse_openapi_json;
/// let json = r#"{
///     "openapi": "3.0.0",
...
/// # Example
/// ```rust,no_run
/// use tauri_appapex_security_auditor_lib::parse_openapi_yaml;
/// let yaml = r#"
/// openapi: "3.0.0"
/// info:
///   title: "My API"
...
fn validate_and_parse_root(root: OpenApiRoot) -> OpenApiResult<OpenApiSpec> {
    ...
    let openapi_version = root.openapi.as_ref().ok_or_else(|| {
        OpenApiError::MissingField("openapi version field".to_string())
    })?;
...
fn extract_endpoints_from_path(path: &str, path_item: &PathItem, endpoints: &mut Vec<DocumentedEndpoint>) {
    let methods = [
        ("GET", &path_item.get),
        ("POST", &path_item.post),
        ("PUT", &path_item.put),
...
/// # Examples
/// ```
/// use tauri_appapex_security_auditor_lib::paths_match;
...
mod tests {
    ...
    fn test_parse_openapi_json() {
        ...
        // Check endpoints
        assert!(spec.endpoints.iter().any(|e| e.path == "/users" && e.method == "GET"));
        assert!(spec.endpoints.iter().any(|e| e.path == "/users" && e.method == "POST"));
        assert!(spec.endpoints.iter().any(|e| e.path == "/users/{id}" && e.method == "GET"));
        assert!(spec.endpoints.iter().any(|e| e.path == "/users/{id}" && e.method == "DELETE"));
    }
...
"#;
    ...
    fn test_parse_openapi_auto_detects_json() {
        let json = r#"{"openapi": "3.0.0", "info": {"title": "Auto API", "version": "1.0"}, "paths": {}}"#;
...
"#;
    ...
    fn test_paths_match() {
        ...
        assert!(paths_match("/users/{id}/posts/{postId}", "/users/123/posts/456"));
    ...
    fn test_swagger_2_rejected() {
        let json = r#"{"swagger": "2.0", "info": {"title": "Test", "version": "1.0"}, "paths": {}}"#;
        let result = parse_openapi_json(json);
    ...
    fn test_openapi_31_supported() {
        let json = r#"{"openapi": "3.1.0", "info": {"title": "Test", "version": "1.0"}, "paths": {}}"#;
        let spec = parse_openapi_json(json).unwrap();
